...
describe('Widget Utilities', () => {
  ...
  describe('Widget Category Validation', () => {
    ...
    it('should ensure all widgets are discoverable through category filters', () => {
      ...
      const totalCategorizedWidgets = vietnamWidgets.length + productivityWidgets.length + 
                                     utilityWidgets.length + personalWidgets.length;
    ...
    it('should ensure no widget is orphaned without a category', () => {
      ...
      const orphanedWidgets = WIDGETS.filter(widget => 
        !validCategories.includes(widget.category)
      );
